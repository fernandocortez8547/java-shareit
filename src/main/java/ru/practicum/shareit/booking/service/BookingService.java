...
public class BookingService {
    ...
    public BookingResponseDto updateBooking(long bookingId, long userId, boolean approved) {
        ...
        if (userId != booking.getItem().getOwner() || booking.getStatus().equals(APPROVED)) {
    ...
    private Collection<Booking> getBookingByStatus(long bookerId, long userId, String status) {
        switch (status) {
            case "ALL":
                return bookingRepository.findByBookerIdOrItemOwner(bookerId, userId);
            case "CURRENT":
                return bookingRepository.findAllByTimeBetween(bookerId, userId);
            case "PAST":
                return bookingRepository.findByBookingByEndInPast(bookerId, userId);
            case "FUTURE":
                return bookingRepository.findByBookingByEndInFuture(bookerId, userId);
            case "WAITING":
